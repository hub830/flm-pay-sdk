...
public class MessageFactory<T extends IsoMessage> {
  ...
  /**
   * Tells the factory to create messages that encode their bitmaps in binary format even when
   * they're encoded as text. Has no effect on binary messages.
   */
  ...
  /**
   * Returns true if the factory is set to create and parse bitmaps in binary format when the
   * messages are encoded as text.
   */
  ...
  /**
   * Tells the receiver to create and parse binary messages if the flag is true. Default is false,
   * that is, create and parse ASCII messages. Sets both binaryHeader and fields to the flag.
   */
  ...
  /**
   * Returns true is the factory is set to create and parse binary messages, false if it uses ASCII
   * messages. Default is false. True if both binaryHeader &amp; binaryFields are set to true
   * 
  ...
  /**
   * Creates a message to respond to a request. Increments the message type by 16, sets all fields
   * from the template if there is one, and copies all values from the request, overwriting fields
   * from the template if they overlap.
   * 
  ...
  public T parseMessage(byte[] buf, int isoHeaderLength, boolean binaryIsoHeader)
      throws ParseException, UnsupportedEncodingException {
    final int minlength =
        isoHeaderLength + (binaryHeader ? 2 : 4) + (binBitmap || binaryHeader ? 8 : 16);
    ...
    if (binaryIsoHeader && isoHeaderLength > 0) {
      ...
      m = createIsoMessage(
          isoHeaderLength > 0 ? new String(buf, 0, isoHeaderLength, encoding) : null);
    }
    m.setCharacterEncoding(encoding);
    final int type;
    if (binaryHeader) {
  ...
  /**
   * Creates a Iso message with the specified binary ISO header. Override this method in the
   * subclass to provide your own implementations of IsoMessage.
   * 
